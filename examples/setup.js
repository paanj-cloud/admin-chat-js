/**
 * Setup Script - Create Test Credentials
 * 
 * This script creates:
 * - Account
 * - Organization
...
async function createSecretKey(accessToken, projectId) {
    const response = await axios.post(
        `${ADMIN_PANEL_URL}/api/projects/${projectId}/api-keys`,
        { keyName: 'Example Secret Key', keyType: 'secret' },
...
async function createPublicKey(accessToken, projectId) {
    const response = await axios.post(
        `${ADMIN_PANEL_URL}/api/projects/${projectId}/api-keys`,
        { keyName: 'Example Public Key', keyType: 'public' },
        { headers: { Authorization: `Bearer ${accessToken}` } }
    );
    return response.data.key;
}
...
async function main() {
    ...
    try {
        ...
        // Create API keys
        const secretKey = await createSecretKey(user.accessToken, project.projectId);
